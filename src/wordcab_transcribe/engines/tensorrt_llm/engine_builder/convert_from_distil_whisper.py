...
def main():
    ...
    parser.add_argument(
        "--output_name", type=str, default="distil-large-v2", help="Output model name"
    )
...
reverse_translation = OrderedDict(
    {
        r"^encoder\.layers\.(\d+)\.self_attn.k_proj\.(\w+)$": (
            r"encoder.blocks.\1.attn.key.\2"
        ),
        r"^encoder\.layers\.(\d+)\.self_attn.out_proj\.(\w+)$": (
            r"encoder.blocks.\1.attn.out.\2"
        ),
        r"^encoder\.layers\.(\d+)\.self_attn.q_proj\.(\w+)$": (
            r"encoder.blocks.\1.attn.query.\2"
        ),
        r"^encoder\.layers\.(\d+)\.self_attn.v_proj\.(\w+)$": (
            r"encoder.blocks.\1.attn.value.\2"
        ),
        r"^encoder\.layers\.(\d+)\.self_attn_layer_norm\.(\w+)$": (
            r"encoder.blocks.\1.attn_ln.\2"
        ),
        r"^encoder\.layers\.(\d+)\.fc1\.(\w+)$": r"encoder.blocks.\1.mlp.0.\2",
        r"^encoder\.layers\.(\d+)\.fc2\.(\w+)$": r"encoder.blocks.\1.mlp.2.\2",
        r"^encoder\.layers\.(\d+)\.final_layer_norm\.(\w+)$": (
            r"encoder.blocks.\1.mlp_ln.\2"
        ),
        ...
        r"^decoder\.layers\.(\d+)\.encoder_attn\.k_proj.(\w+)$": (
            r"decoder.blocks.\1.cross_attn.key.\2"
        ),
        r"^decoder\.layers\.(\d+)\.encoder_attn\.out_proj.(\w+)$": (
            r"decoder.blocks.\1.cross_attn.out.\2"
        ),
        r"^decoder\.layers\.(\d+)\.encoder_attn\.q_proj.(\w+)$": (
            r"decoder.blocks.\1.cross_attn.query.\2"
        ),
        r"^decoder\.layers\.(\d+)\.encoder_attn\.v_proj.(\w+)$": (
            r"decoder.blocks.\1.cross_attn.value.\2"
        ),
        r"^decoder\.layers\.(\d+)\.encoder_attn_layer_norm\.(\w+)$": (
            r"decoder.blocks.\1.cross_attn_ln.\2"
        ),
        r"^decoder\.layers\.(\d+)\.self_attn\.k_proj\.(\w+)$": (
            r"decoder.blocks.\1.attn.key.\2"
        ),
        r"^decoder\.layers\.(\d+)\.self_attn\.out_proj\.(\w+)$": (
            r"decoder.blocks.\1.attn.out.\2"
        ),
        r"^decoder\.layers\.(\d+)\.self_attn\.q_proj\.(\w+)$": (
            r"decoder.blocks.\1.attn.query.\2"
        ),
        r"^decoder\.layers\.(\d+)\.self_attn\.v_proj\.(\w+)$": (
            r"decoder.blocks.\1.attn.value.\2"
        ),
        r"^decoder\.layers\.(\d+)\.self_attn_layer_norm\.(\w+)$": (
            r"decoder.blocks.\1.attn_ln.\2"
        ),
        r"^decoder\.layers\.(\d+)\.fc1\.(\w+)$": r"decoder.blocks.\1.mlp.0.\2",
        r"^decoder\.layers\.(\d+)\.fc2\.(\w+)$": r"decoder.blocks.\1.mlp.2.\2",
        r"^decoder\.layers\.(\d+)\.final_layer_norm\.(\w+)$": (
            r"decoder.blocks.\1.mlp_ln.\2"
        ),
    }
)
